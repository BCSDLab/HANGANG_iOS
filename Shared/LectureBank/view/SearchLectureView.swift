...
struct SearchLectureView: View {
    @EnvironmentObject var uploadLectureViewModel: UploadLectureBankViewModel
    @ObservedObject var viewModel: SearchLectureViewModel
    ...
    init(query: String) {
        ...
        self.viewModel = SearchLectureViewModel(
        initQuery: query)
    }
            ...
            VStack {
                ...
                VStack(alignment: .center, spacing: 0){
                    ...
                    List(self.viewModel.lectureResult, id: \.self) { lecture in
                        Button(action: {
                            if(self.uploadLectureViewModel.selectedLecture == lecture) {
                                self.uploadLectureViewModel.selectedLecture = nil
                            } else {
                                self.uploadLectureViewModel.selectedLecture = lecture
                            }
                        }) {
                            SearchLectureItem(lecture: lecture)
...
struct SearchLectureView_Previews: PreviewProvider {
    static var previews: some View {
        SearchLectureView(query: "")
    }
}
