...
struct SignUpView: View {
    ...
    var body: some View {
        VStack(alignment: .leading){
            ...
            Spacer()
            NavigationLink(destination: MajorView(password: self.viewModel.password, email: self.viewModel.email, nickname: self.viewModel.nickname), isActive: .constant(self.viewModel.allChecked)) {
                HStack{
                    Spacer()
                    Text("다음")
                        .font(.system(size: 14, weight: .medium))
